/**
 * https://github.com/partykit/partykit/blob/main/packages/y-partykit/src/chunking.ts
 */
 ...
 */
export const sendChunked = (data: ArrayBufferLike, ws: WebSocket, chunkSize = CHUNK_MAX_SIZE) => {
  ...
  if (!warnedAboutLargeMessage) {
    console.warn(
      `Message size exceeds ${chunkSize}, which is the maximum size for an individual message. The message will be split into chunks.`,
